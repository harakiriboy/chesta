...
namespace Chesta.Infrastructure.Persistence.Repositories
{
    public class GenericRepository<TEntity> : IGenericRepository<TEntity> where TEntity : class
    {
        ...
        public async Task<IEnumerable<TItem>> GetAllByIdAsync<TItem>(ASpec<TEntity> spec)
        {
            var res = await _context.Set<TEntity>().Where(spec).ProjectToType<TItem>().AsQueryable().ToListAsync();
            return res;
        }
    }
}